...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const x1212 = Math.abs(x2 - x1);
  const y1212 = Math.abs(y2 - y1);
  const oio = x1212 * x1212 + y1212 * y1212;
  return Math.sqrt(oio);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const pol34 = x1 * x2 + y1 * y2;
  const de1 = Math.sqrt(x1 * x1 + y1 * y1);
  const de2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(pol34 / (de1 * de2));
}
...
function roundToPowerOfTen(num, pow) {
  const num34 = num / 10 ** pow;
  return Math.round(num34) * 10 ** pow;
}
...
function getFloatOnString(str) {
  const str34 = Number.parseFloat(str);
  if (Number.isNaN(str34)) {
    return NaN;
  }
  return str34;
}
...
function getIntegerOnString(str, base) {
  const str34 = Number.parseInt(str, base);
  if (Number.isNaN(str34)) {
    return NaN;
  }
  return str34;
...
function getCountOfOddNumbers(number) {
  ...
  return Math.abs(Math.trunc(number / 2)) + 1;
}
